...
"""Temporary DAG that deletes Pathways worker pods left behind by a slow GC.

TODO(b/563221051): delete this file and its whitelist entry.
Only needed while the cluster's garbage collector is too slow to remove the
pods itself. The DAG has no other purpose, so it should be reverted wholesale
...
to the pods. On `bodaborg-v5p-nap` the GC is intermittently saturated by a
co-tenant, and while it is, that cascade sits in its queue: measured delays of
47m09s, 15m07s, 11m17s and 10m36s, against well under a minute when the GC is
healthy. Until the cascade is processed the worker pods stay in phase Running
and keep the v5p chips busy.
...
deletion driven by ownerReferences. Deleting pods is the only thing this DAG
does; it does not cancel jobs, touch JobSets, or free any other resource.
...
# TODO(b/563221051): remove this schedule together with the file.
# Polling only exists to beat the slow garbage collector; once the GC keeps up
...
@task
def delete_stranded_pathways_pods(dry_run: str = "False") -> int:
  ...
  # Log the pre-filter counts too. Without them "0 matched" is ambiguous: it
  # looks identical whether the cluster genuinely has no post-training
  # workload right now or the query itself is broken (wrong namespace, revoked
  # RBAC, renamed label). This DAG's main failure mode is doing nothing
  # quietly, so the raw count is the signal that it is still wired up.
  logging.info(
      "Listed %d pod(s) across %d JobSet(s) in %s/%s. After filtering on %s: "
      "%d pod(s) across %d workload(s).",
      len(pod_list.items),
      len(
          {
              (p.metadata.labels or {}).get(_JOBSET_NAME_LABEL)
  ...
  logging.info("Deleted %d stranded pod(s).", deleted)
  return deleted


with models.DAG(
    dag_id="delete_stranded_pathways_pods",
    schedule=SCHEDULED_TIME,
    # `temporary` surfaces the workaround in the Airflow UI. See the TODO at
    # the top of this file.
    tags=["multipod_team", "clean_up", "pathways", "TPU", "temporary"],
  ...
  delete_stranded_pathways_pods(dry_run="{{ params.dry_run }}")
