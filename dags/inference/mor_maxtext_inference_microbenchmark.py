...
from dags.inference.configs import mor_maxtext_inference_microbenchmark_gce_config as maxtext_inference_microbenchmark_gce_config
...
BASE_OUTPUT_DIRECTORY = (
    "gs://inference-benchmarks/logs/mor-maxtext-inference-microbenchmark"
)
...
def generate_model_configs(
  ...
  test_run_tag = f"{model_config_name}-bs{per_device_batch_size}-{compute_axis_order_tag}-vm{vm_number}"
...
with models.DAG(
    dag_id="mor-maxtext-inference-microbenchmark",
    tags=["mor", "inference_team", "maxtext", "microbenchmark"],
    start_date=datetime.datetime(2024, 1, 19),
    schedule=None,
    catchup=False,
) as dag:
  test_name_prefix = "mor-max-micro"
  ...
  test_templates = {
      LLAMA2_7B: {
          "maxtext_branch": "-b mor--layout-sweep-w-compute-axis-order",
          "sleep_time": 60,
