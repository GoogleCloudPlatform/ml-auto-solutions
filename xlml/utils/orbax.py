"""Utility functions and Airflow tasks for managing Orbax CheckpointConfiguration.
This module provides tasks for creating, applying, and deleting
CheckpointConfiguration objects on a Kubernetes cluster.
"""
...
def apply_cpc(
  ...
  try:
    ...
    if api_error.status == 404:
      ...
      logging.error(
          f"Error applying CheckpointConfiguration:"
...
def delete_cpc(
  ...
  try:
    ...
    logging.info(
        f"Waiting indefinitely for CheckpointConfiguration"
    ...
    if e.status == 404:
      logging.info(
          ...
          f"Already deleted or never existed. Skipping deletion."
      )
      return
    else:
      logging.error(
          f"Error during initial delete request for CheckpointConfiguration: "
          f"{e.status} - {e.reason} - {e.body}"
      )
      raise AirflowFailException(
