...
def validate_log_with_step(
  ...
  """
  Checks the steps are saved (locally) correctly.
  Compared with the expected steps.
  Args:
...
def validate_log_with_gcs(
  ...
  """This task will compare last recorded backup
  step stored by replicator with last step stored in gcs.
  Make sure replicator work correctly
  """
  entries = list_log_entries(
      project_id=project_id,
      location=location,
  ...
  for entry in entries:
    if entry.payload is not None:
      ...
      for line in payload_str.split("\n"):
        ...
        # If could not found those valuses
        if match_gcs and match_step:
          ...
          # Add it to a global list to later to compare.
          gcs_save_step_list.append(int(step))
        else:
          return False

  # Compare last step found in replicator.
  # The logs and last step extracted from filename bucket
  if len(gcs_save_step_list_bucket) > 0 and len(gcs_save_step_list) > 0:
    # Extract s60 from  file name eg. maxtext-s60-n14-w0.meta
    ...
    if match is None:
      raise AirflowFailException(
          "Could not extract step from filename: %s", raw_str_filename
      )
