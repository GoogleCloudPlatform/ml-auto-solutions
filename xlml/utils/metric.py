...
def get_gcs_profile_locations(dir_location: str) -> str:
  """
...
def download_profile(
  ...
  Create tmp_dir = f"/home/airflow/gcs/data/profile-{process_id}"
  Download profile to tmp_location = f"{tmp_dir}/gke.xplane.pb"

  Returns:
    tmp_dir, or "" if no match
  """
  ...
  # unique directory to accomodate intermediate files from profile extraction
  tmp_dir = f"/home/airflow/gcs/data/profile-{process_id}"
...
def xplane_to_metrics(input_dir: str) -> dict:
  """
  Extract metrics from xplane file `{input_dir}/gke.xplane.pb`.
  Remove temporory input_dir generated by `download_profile`.

  Returns:
    A dictionary of profile metrics, or None if input_dir is ""
  """
  ...
  def cleanup(input_dir):
    # remove temporary directory generated by `download_profile`
    if input_dir.startswith("/home/airflow/gcs/data/profile-"):
  ...
  def xplane_tool(input_path: str, tool: str, params: dict) -> dict:
  ...
  if "overview_page" not in available_tools:
    ...
    try:
      overview_page = xplane_tool(input_path, tool="overview_page", params={})
      out.update({
  ...
  if "op_profile" not in available_tools:
    ...
    return out
  try:
    op_profile = xplane_tool(input_path, tool="op_profile", params={})
    out.update({
