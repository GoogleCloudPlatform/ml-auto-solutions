...
class XpkTask(BaseTask):
  ...
  def run_with_name_gen_and_quarantine(
    ...
    if QuarantineTests.is_quarantined(test_name):
      with quarantine_task_group:
        return self.run_with_run_name_generation(use_pathways, xpk_branch, run_name_env, nested_run_name_in_tb_file_location)
    else:
      return self.run_with_run_name_generation(use_pathways, xpk_branch, run_name_env, nested_run_name_in_tb_file_location)

  def run_with_run_name_generation(
      self, use_pathways: bool = False, xpk_branch: str = xpk.MAIN_BRANCH, run_name_env: str = "M_RUN_NAME",
      nested_run_name_in_tb_file_location: bool = True
  ) -> DAGNode:
    ...
    with TaskGroup(
      ...
      tb_file_location = name_format.generate_tb_file_location(
          run_name, self.task_metric_config.tensorboard_summary.file_location, nested_run_name_in_tb_file_location
      )
