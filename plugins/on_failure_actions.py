...
class DagRunListener:
  ...
  def on_dag_run_failed(self, dag_run: DagRun, msg: str):
    ...
    try:
      ...
      failed_tasks_list = ", ".join(failed_task_ids)
      title = (
          f"[{self.log_prefix}] {dag_run.dag_id} [{failed_tasks_list}] failed"
      )
      body += "---\n"
      ...
      logging.error(f"[{self.log_prefix}] over")
    except AirflowException as airflow_e:
      logging.error(f"[{self.log_prefix}] Airflow exception: {airflow_e}")
    except Exception as e:
      logging.error(f"[{self.log_prefix}] Unexpected exception: {e}")
