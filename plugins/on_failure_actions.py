...
class DagRunListener:
  ...
  def get_group_id(task_instance: TaskInstance):
    ...
    if group_id:
      return (
          group_id.split(".")[0] if len(group_id.split(".")) > 0 else group_id
      )
    else:
      return task_id
  ...
  def create_issue(self, client, title, body, assignees):
  ...
  def update_issue(self, issue, body, assignees):
    logging.error(f"[{self.log_prefix}] Update the latest one")
    updated_body = issue.body + body
    issue.edit(body=updated_body, assignees=assignees)

  @hookimpl
  def on_dag_run_failed(self, dag_run: DagRun, msg: str):
    logging.error(f"[{self.log_prefix}] DAG run: {dag_run.dag_id} failed")
    logging.error(f"[{self.log_prefix}] msg: {msg}")
    ...
    try:
      ...
      for ti in dag_run.task_instances:
        if ti.state != "failed":
          continue
        logging.error(
            f"[{self.log_prefix}] Task {ti.dag_id}-{ti.task_id} failed"
        )
        ...
        if not first_failed_group_id:
          first_failed_group_id = self.get_group_id(ti)
        body += f"- **Failed Task**: [{ti.task_id}]({link})\n"
        if ti.task.owner and ti.task.owner != "airflow":
          assignees.append(ti.task.owner)
      title = (
          f"[{self.log_prefix}] {dag_run.dag_id} {first_failed_group_id} failed"
      )
      body += "---\n"
      logging.error(f"[{self.log_prefix}] assignees: {assignees}")
      client = self.get_github_client()
      issue = self.query_latest_issues(client, title)
      try:
        if issue:
          self.update_issue(issue, body, assignees)
        else:
          self.create_issue(client, title, body, assignees)
        ...
        if "422" in str(e):
          ...
          if issue:
            self.update_issue(issue, body, [])
          else:
            self.create_issue(client, title, body, [])
        else:
          raise e
